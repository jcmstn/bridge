...
| `uv run python dc/dc_rt_log_tui.py` | R vs T log — fixed ±I reversal (6221 + 2182) sampled back to back while the temperature drifts freely; MercuryiTC read only, before and after each sample (`temperature_N_drift_K` per point), sensors probed verbosely at connect with a 2 → 1 fallback; stops on Stop / max duration / a T crossing, Stop saves as `completed` (type `RT`) — Textual, also a web page (`/dc/rt-log`) |
