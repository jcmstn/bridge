...
| `NLSW` | Nonlocal spin-current switching, 6221 + 2182A only — field-initialized sweep of single-lobe (0 → ±I → 0) WAVE write pulses + DC reversal-averaged nonlocal read; key axis = the init magnet current (`current_A`) when the run initializes the state (`sot/sot_nonlocal_switching`) |
