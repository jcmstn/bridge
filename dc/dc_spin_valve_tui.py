...
class RunScreen(Screen):
    ...
    def _save_run_png(self, ctx: RunContext, iter_records: list[dict]) -> None:
        """One PNG per run (own run number), as if each sense current / gate
        voltage had been started by hand -- no combined overlay."""
        try:
    ...
    def _on_status_comment(self, result: Optional[tuple[str, str]]) -> None:
        # With several sense currents / gate voltages the runs before the
        # last were implicitly "skipped" -- left at the outcome status
        # do_run() wrote right after each one, with no comment. Only the
        # last run, the one the operator is looking at, gets the
        # status/comment they entered.
