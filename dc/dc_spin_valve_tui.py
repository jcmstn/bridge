...
def _save_measurement_png(records: list[dict], png_path: Path,
    ...
    """`plan`/`comment` add a small "at a glance" text annotation -- a
    single fixed sense current and/or gate voltage (only when that value
    isn't already this run's key_axis filename token -- see
    MeasurementPlan.series_values), plus the operator's comment. Called
    once when the run ends (comment="") and again, to overwrite the PNG in
    place, once the operator's comment is known."""
    if not records:
        return
