...
def build_summary(state: dict) -> tuple[list[str], list[str], list[str]]:
    ...
    if not 0 < I <= _AC_CURRENT_CEILING_A:
        ...
        info.append(f"Excitation current I = {format_si(I, 'A')} peak "
...
def _save_measurement_png(records: list[dict], png_path: Path,
    ...
    direction, the AC excitation current, the operator's comment) for
    context not already in the filename. Called once when the run ends
    ...
    if plan is not None:
        ...
        amp_A = plan.header_extra.get("excitation_amplitude_A")
        if freq_Hz is not None and amp_A is not None:
            lines.append(f"AC excitation: {format_si(amp_A, 'A')} @ {format_si(freq_Hz, 'Hz')}")
    if comment:
...
class RunScreen(Screen):
    ...
    def do_run(self) -> None:
        ...
        try:
            ...
            self._set_status_threadsafe("Starting 6221 AC current source …")
            source = connect_ac_source(plan.ac_cfg)

            self._set_status_threadsafe("Locking MFLI oscillators to the 6221 marker (ExtRef) …")
...
class MFLIDualHarmonic6221App(App):
    ...
    def _build_plan(self, state: dict) -> MeasurementPlan:
        ...
        return MeasurementPlan(
            ...
            ac_cfg=ac_cfg, leader_extref_cfg=leader_extref_cfg,
